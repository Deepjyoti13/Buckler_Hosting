...
def index(request):
    ...
    feature = Feature.objects.last()
    # men = Category.objects.get(title = 'Men')
    # child = men.get_children()
    deal_day = Product.objects.filter(dealActive=True).filter(deals='Deal of the day').last()
    deal_week = Product.objects.filter(dealActive=True).filter(deals='Deal of the week').last()
    deal_month = Product.objects.filter(dealActive=True).filter(deals='Deal of the month').last()
    print(deal_day)
    # print(request.user.email)
    # deal = Deal.objects.filter(active=True)
    product_slider = Category.objects.filter(active = True).filter(carousel = True)
    catspecialdeal = Category.objects.filter(active = True).filter(special_deal = True)
    ...
    context = {'setting': setting, 'page': page, 'category': category, 'product_slider': product_slider, 'range': range(
        1, len(product_slider)), 'product_picked': product_picked, 'products': products, 'deal_day': deal_day, 'deal_week': deal_week, 'deal_month': deal_month, 'catspecialdeal': catspecialdeal,
        'feature': feature,
               # 'men': men, 'child': child
               }
...
def t_n_c(request):
    ...
    return render(request, 'eshop/t-and-c.html', context)
