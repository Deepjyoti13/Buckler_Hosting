...
def payu_success(request):
    ...
    message = render_to_string('order/email.html', {
                'user': data["firstname"],
                'ordercode': data["txnid"],
                'status': "Successful (via Prepay)!",
                'para': "Thank you for ordering. Keep shopping!"
            })
    to_email = data["email"]
    send_mail(mail_subject, message, 'expresstotell@gmail.com', [to_email], fail_silently=False,)
    print(send_mail)
    context = {
        'ordercode': data["txnid"], 'name': data["firstname"], 
    }
...
def payu_failure(request):
    ...
    data = {k: v[0] for k, v in dict(request.POST).items()}
    response = payu.verify_transaction(data)
    mail_subject = 'Order Failed.'
    message = render_to_string('order/email.html', {
                'user': data["firstname"],
                'ordercode': data["txnid"],
                'status': "Failed (via Prepay)!",
                'para': "Kindly re-initiate your payment and enter valid credentials."
            })
    to_email = data["email"]
    send_mail(mail_subject, message, 'expresstotell@gmail.com', [to_email], fail_silently=False,)
    context = {
    ...
    return render(request, 'order/order_failed.html', context)
    ...
    if request.method == 'POST':  # if there is a post
        ...
        if form.is_valid():
            ...
            message = render_to_string('order/email.html', {
                        ...
                        'ordercode': ordercode,
                        'status': "Successful (via COD)!",
                        'para': "Thank you for ordering. Keep shopping!"
                    })
            to_email = request.user.email
            send_mail(mail_subject, message, 'expresstotell@gmail.com', [to_email], fail_silently=False,)
            return render(request, 'order/cod_completed.html',{'ordercode':ordercode,'category': category, 'name': data.first_name })
        else:
            messages.warning(request, form.errors)
            return HttpResponseRedirect("/order/orderproduct")
